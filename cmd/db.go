...
func queryCount(table *table, db *sql.DB) (int, error) {
	rows, err := db.Query(fmt.Sprintf("SELECT COUNT(*) as count FROM %s.%s", table.schema, table.name))
	if err != nil {
...
func genDeleteSqls(table *table, db *sql.DB, count int) ([]string, [][]interface{}, error) {
	...
	rows, err := db.Query(fmt.Sprintf("SELECT * FROM %s.%s limit %d, %d", table.schema, table.name, start, count))
	if err != nil {
		return nil, nil, errors.Trace(err)
	}
	...
	for rows.Next() {
		...
		sqls = append(sqls, fmt.Sprintf("delete from %s.%s where %s", table.schema, table.name, where))
...
func genUpdateSqls(table *table, db *sql.DB, count int) ([]string, [][]interface{}, error) {
	...
	for rows.Next() {
		...
		sqls = append(sqls, fmt.Sprintf("update %s.%s set `%s` = %s where %s", table.schema, table.name, column.name, updateData, where))
		args = append(args, data)
	}
...
func execSQLs(db *sql.DB, schema string, sqls []string) error {
	for _, sql := range sqls {
		err := execSQL(db, schema, sql)
		if err != nil {
			return errors.Trace(err)
		}
	}
...
func execSQL(db *sql.DB, schema string, sql string) error {
	if len(sql) == 0 {
		return nil
	}
